...
def dd_items(lat, lon, address, muni, pref):
    ...
    return [
        ...
        ("12","森林","",search(f"{p} {m} 森林情報 GIS 地域森林計画対象民有林"),"リンク,スクショ","都道府県の森林GISで地域森林計画対象民有林・保安林を確認。"),
        ("12-2","保安林（参考）","",search(f"{p} {m} 保安林 指定状況 位置図 森林GIS"),"リンク,スクショ","【参考値・APIなし／自動判定不可】reinfolibに保安林レイヤが無く自動判定できません。都道府県の森林GIS・保安林（指定・解除）位置図で、対象地が保安林（水源かん養・土砂流出防備・土砂崩壊防備等）に該当するか目視確認。保安林は原則開発不可で、立木の伐採・土地の形質変更には都道府県知事の許可が必要（許認可No.34と連動）。該当が疑われる場合は都道府県 森林部局／森林管理署へ照会。"),
