...
- **2回目以降が高速**：reinfolibの応答を**タイル(z/x/y)単位で永続キャッシュ**するため、近隣案件（同一タイル≒1〜2km）の再判定はネット取得を省いて即応答します。農地(A12)/森林(A13)も県単位でキャッシュ・索引化します
...
4. **Environment** で以下の環境変数を設定：
   ...
   | `REINFO_CACHE_TTL_DAYS` | 任意 | reinfolibタイルキャッシュの有効日数（既定 `30`）。この日数を過ぎたタイルは次回リクエスト時に再取得する。空欄なら既定値 |

5. **Create Web Service** → 数分でビルド完了 → `https://<名前>.onrender.com` で社内公開

> APIキーやパスワードは**コードに書かず、必ずRenderの環境変数（Environment）に設定**してください。
> `render.yaml` は農地(A12)・森林(A13)・reinfolibタイルキャッシュ用の永続ディスク（1GB, `/var/data`）を定義しています。
